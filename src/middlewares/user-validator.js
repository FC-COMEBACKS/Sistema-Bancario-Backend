...
export const validatorRegister = [
  ...
  body("username").notEmpty().withMessage("El username es obligatorio").custom(usernameExists),
  body("password").notEmpty().withMessage("La contraseña debe contener al menos una mayúscula, una minúscula y un número"),
  body("dpi").notEmpty().withMessage("El DPI es obligatorio").custom(dpiExists),
  body("email").notEmpty().withMessage("El email es obligatorio").isEmail().withMessage("No es un email válido").custom(emailExists),
  ...
  body("ingresosMensuales").notEmpty().withMessage("Los ingresos mensuales son obligatorios").isNumeric().withMessage("Los ingresos mensuales deben ser numéricos").isFloat({ min: 100 }).withMessage("Los ingresos mensuales deben ser al menos 100"),
  validateField,
  handleErrors
];
