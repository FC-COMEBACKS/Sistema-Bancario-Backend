    ...
    try {
        ...
        if (!esAdmin && !esMismoUsuario) {
            return res.status(403).json({
                success: false,
                message: "No tiene autorización para acceder a la información de este usuario"
            });
        }
...
export const getUsers = async (req, res) => {
    try {
        ...
        if (usuario.rol !== 'ADMIN') {
            return res.status(403).json({
                success: false,
                message: "No tiene autorización para acceder al listado de usuarios. Esta función está limitada a administradores."
            });
        }
...
export const deleteUserAdmin = async (req, res) => {
    try {
        ...
        const user = await Usuario.findByIdAndUpdate(uid, { estado: "INACTIVO" }, { new: true });
...
export const deleteUserClient = async (req, res) => {
    try {
        ...
        const user = await Usuario.findByIdAndUpdate(usuario._id, { estado: "INACTIVO" }, { new: true });
...
export const updateUserUser = async (req, res) => {
    try {
        ...
        delete data.dpi;
        delete data.rol;
        const updatedUser = await Usuario.findByIdAndUpdate(usuario._id, data, { new: true });
        return res.status(200).json({
            success: true,
            msg: 'Usuario Actualizado',
            user: updatedUser,
...
export const updateRole = async (req, res) => {
    try {
        ...
        const updatedUser = await Usuario.findByIdAndUpdate(uid, { rol: newRole }, { new: true });
        return res.status(200).json({
            success: true,
            msg: 'Rol actualizado',
            user: updatedUser,
