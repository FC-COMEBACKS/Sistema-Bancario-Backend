...
const verificarLimiteDiarioTransferencias = async (cuentaId, monto) => {
  ...
  if (totalTransferidoHoy + monto > 10000) {
    throw new Error(
      "Has excedido el límite diario de transferencia de Q10,000"
    );
  }
...
export const getDetallesCuenta = async (req, res) => {
  try {
    ...
    const esPropietario =
      cuenta.usuario._id.toString() === usuario._id.toString();
...
export const realizarTransferencia = async (req, res) => {
  try {
    ...
    const movimiento = new Movimiento({
      ...
      descripcion:
        descripcion ||
        `Transferencia de ${origen.numeroCuenta} a ${destino.numeroCuenta}`,
      fechaHora: new Date(),
    });
...
export const getMovimientosCuenta = async (req, res) => {
  try {
    ...
    const cuenta = await Cuenta.findById(cid).populate({
      path: "movimientos",
      options: {
