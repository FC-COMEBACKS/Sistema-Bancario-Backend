...
export const transferirAFavorito = async (req, res) => {
    try {
        const { favoritoId, monto, descripcion = "Transferencia a favorito" } = req.body;
        const usuarioId = req.usuario._id;
        ...
        if (cuentaOrigen.movimientos) cuentaOrigen.movimientos.push(nuevoMovimiento._id);
        if (cuentaDestino.movimientos) cuentaDestino.movimientos.push(nuevoMovimiento._id);
