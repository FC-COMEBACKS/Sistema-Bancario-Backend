...
export const getMovimientos = async (req, res) => {
  try {
    ...
    if (cuenta) {
      filtro.$or = [{ cuentaOrigen: cuenta }, { cuentaDestino: cuenta }];
    }
      ...
      return {
        ...
        cuentaOrigen: mov.cuentaOrigen
          ? {
              ...
              titular: mov.cuentaOrigen.usuario
                ? mov.cuentaOrigen.usuario.nombre
                : "No especificado",
            }
          : null,
        cuentaDestino: mov.cuentaDestino
          ? {
              ...
              titular: mov.cuentaDestino.usuario
                ? mov.cuentaDestino.usuario.nombre
                : "No especificado",
            }
          : null,
      };
    ...
    const resultado = {
      ...
      cuentaOrigen: movimiento.cuentaOrigen
        ? {
            ...
            titular: movimiento.cuentaOrigen.usuario
              ? movimiento.cuentaOrigen.usuario.nombre
              : "No especificado",
          }
        : null,
      cuentaDestino: movimiento.cuentaDestino
        ? {
            ...
            titular: movimiento.cuentaDestino.usuario
              ? movimiento.cuentaDestino.usuario.nombre
              : "No especificado",
          }
        : null,
    };
...
export const realizarTransferencia = async (req, res) => {
  try {
    ...
    const cuentaOrigenObj = await Cuenta.findOne({
    ...
    const usuarioOrigenId = cuentaOrigenObj.usuario._id
      ? cuentaOrigenObj.usuario._id.toString()
      : cuentaOrigenObj.usuario.toString();
    ...
    const cuentaDestinoObj = await Cuenta.findOne({
    ...
    await Promise.all([cuentaOrigenObj.save(), cuentaDestinoObj.save()]);
    ...
    const resultado = {
      ...
      cuentaOrigen: {
        ...
        titular: cuentaOrigenObj.usuario
          ? cuentaOrigenObj.usuario.nombre
          : "No especificado",
      },
      cuentaDestino: {
        id: cuentaDestinoObj._id,
        numeroCuenta: cuentaDestinoObj.numeroCuenta,
        titular: cuentaDestinoObj.usuario
          ? cuentaDestinoObj.usuario.nombre
          : "No especificado",
      },
...
export const revertirDeposito = async (req, res) => {
  try {
    ...
    const cuenta = await Cuenta.findById(deposito.cuentaDestino).populate(
      "usuario",
      "nombre"
    );
    ...
    await Promise.all([cuenta.save(), deposito.save()]);
...
export const comprarProducto = async (req, res) => {
  try {
    const {
      numeroCuenta,
      productoId,
      descripcion = "Compra de producto/servicio",
    } = req.body;

    const cuenta = await Cuenta.findOne({ numeroCuenta }).populate(
      "usuario",
      "nombre"
    );
